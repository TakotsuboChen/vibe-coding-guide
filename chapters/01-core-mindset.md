...
一个常见的错误期望，是以为 Claude Code 像数据库一样自动同步状态。真相是：
...
**这套体系的运转，靠的是你主动维护的几个文件 + 几个机械习惯，而不是任何"自动魔法"。** 这些习惯不依赖代码能力，纯是纪律，初学者完全可以做到。

## "做什么"你驱动，"怎么做"Claude 自主

理解 Claude Code 的关键是：它是一个 **agent（代理）**，不是问一句答一句的聊天机器人。它的 agentic loop 是：
